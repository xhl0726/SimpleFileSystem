...
void my_ls()
{
    ...
    for(i = 0; i < rbn / sizeof(fcb); i++)
    {
        if(fcbptr->free)
        {
			...
			(fcbptr->date >> 9) + 1980, (fcbptr->date >> 5) & '1''0f, fcbptr->date & '1''1f, 
			fcbptr->time >> 11, (fcbptr->time >> 5) & '1''3f, fcbptr->time & '1''1f * 2);
            else
                printf("%s.%s\t\t%dB\t\t%d/%d/%d\t%02d:%02d:%02d\t\n", fcbptr->filename, 
			fcbptr->exname, (int)(fcbptr->length), (fcbptr->date >> 9) + 1980, 
			(fcbptr->date >> 5) & '1''0f, fcbptr->date & 0x1f, fcbptr->time >> 11, 
...
void my_create(char *filename)
{
    ...
    do_write(curdir, (char *)fcbptr, sizeof(fcb), 2);
    fcbptr = (fcb *)text;
    fcbptr->length = penfilelist[curdir].length;
    openfilelist[curdir].count = 0;
    do_write(curdir, (char *)fcbptr, sizeof(fcb), 2);
    openfilelist[curdir].fcbstate = 1;
}
